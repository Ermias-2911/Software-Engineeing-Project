...
def results():
    ...
    if request.method == "POST":
        ...
        if filter is None:
            ...
            roomQ = ''
            if varData['room']:
                roomQ = 'rent_type_id = 1'

            apartmentQ = ''
            if varData['apartment']:
                apartmentQ = 'rent_type_id = 2'
            ...
            return render_template('results.html', listings=cursor.fetchall(), message=filterQuery, persistence=persistanceList)
...
def more_info_message():
    if 'username' in session:
        if request.method =='POST':
            ...
            #insertDB('message_table', ('body', 'sender_user_id', 'reciever_user_id', 'listing_id'), (body, loginuserid, senderid, listing_id))
            insertDB('message_table', ('body', 'sender_user_id', 'reciever_user_id', 'listing_id'), (body, loginuserid, sender_id, listing_id))
...
def user_messages():
    if 'username' in session:
        ...
        try:
            ...
            senderIDName = 0
            return render_template('/user_messages_empty.html')
